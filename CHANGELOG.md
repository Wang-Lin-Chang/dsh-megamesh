...
- ARENA 真模型擂台（E41）：DeepSeek 双模型（v4-flash/v4-pro）真 API 证词生成 → 军法结构审吃真实输出 → 三票制擂台 winner——协议从 mock 走向真模型战场。幻觉黄金标准对照（不可知问题"2027 诺贝尔物理学奖得主"）：单模型诚实答"无人知晓"，两模型同样诚实——真实数据显示该注入下模型不幻觉（数据不造假，实验如实记录）。

## [0.22.0] - 2026-08-17

### Added

- 推理竞技场 ARENA（E40，自设计框架——模型不当法官当证人，结论打赢擂台才值得信）：六层复用已实测底盘（证词立场+信用 / 军法只查结构不查语义 / 三票制擂台（chair+自洽+离群双非 LLM 防线）/ 平行宇宙翻案 / 裁判任期治理 / 成本计量内建）。两个 TREMOR 没有的维度：①被否结论进休眠分支可复活（时间机器语义）②验证者误判两次降级清零（裁判也被治理）。诚实边界：同源 Transformer 共同幻觉的对策=非 LLM 防线；语义蕴含不冒充（交质证层软判断）。

## [0.21.0] - 2026-08-17

### Added

- 审计塔 φ 衰减命题实测（E39）：AIGS 自指方程 𝓡²=𝓡+𝓘 特征值 φ 的工程验证——① 解析边界：φ 衰减 ⟺ 每层检出率 p=1/φ²≈0.382（等价条件，可证伪）；② 结构验证：三级自指=审计塔三层真实跑通（E36 两层扩展三层）；③ 系数实测：衰减比=1-p（实测 0.312 vs 解析 0.300 @ p=0.7）；④ 特例验证：p=0.382 时实测衰减比 0.616 vs 1/φ 0.618。诚实判决：结构层落地、φ 系数衰减是特例非必然——真实缺陷集检出率实测若为 0.382 才从特例变事实。
